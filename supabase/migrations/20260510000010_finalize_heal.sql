-- 20260510000010_finalize_heal.sql
-- Apply on_battle_end heal effects in finalize_battle.
-- Same pattern as xp_bonus: server aggregates from deck, applies to final HP.
...
begin
  ...
  -- on_battle_end heal aggregation (server-authoritative, mirrors xp_bonus pattern)
  ...
  -- Mutate last log entry to reflect post-heal HP (so result page replay reads correctly)
  v_log := p_log;
